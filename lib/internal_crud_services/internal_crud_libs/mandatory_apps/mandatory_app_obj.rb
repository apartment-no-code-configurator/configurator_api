...
module InternalCrudServices
  ...
  module InternalCrudLibs
    ...
    module MandatoryApps

      class MandatoryAppObj

        include InternalCrudServices::CrudOperationsHelpers

        @@base_internal_service_helper = BaseInternalServiceHelper.new(MandatoryApp)

        attr_accessor :params, :join_internal_service_helper, :parent_record, :record
        def initialize(params={})
          @params = params
          @record = params[:id].present? ? get_object : MandatoryApp.new(params)
        end
